...
// asymmetric switching scenario 1; faster switching from scarce  to abuundant resource supply
//  Createdon 2. 11. 19
//
//
...
#define xmax 200// max resource supply
#define xmin 50// min resource supply
#define xmean (xmax+xmin)/2 // mean resource supply
...
#define asym 1.2// mupliplier for asymmetirc switching
...
double SumTransition(int x, int y1, int y2, int z, int xi,  double mu1, double Kx1, double d1, double Kz1, double mu2,double Kx2 ,double d2, double Kz2, double nu)
{
    ...
    double sum;
    int i;
    // note faster switching rate from xi=1 to xi=-1
   if (xi==1)
   {
       prob[0]=dilution*xmax;// increase of resoruce with max supply
       prob[8]=nu;//switching resource
   }
   else
   {
       prob[0]=dilution*xmin;// increase of resoruce with min supply
       prob[8]=asym*nu;//switching resource
   }
...
int *Transition(int x, int y1, int y2, int z, int xi,  double mu1, double Kx1, double d1, double Kz1, double mu2,double Kx2 ,double d2, double Kz2, double nu)
{
   ...
   {
       prob[0]=dilution*xmax;// increase of resoruce with max supply
       prob[8]=nu;//asymmetric switching resource
   }
   else
   {
       prob[0]=dilution*xmin;// increase of resoruce with min supply
       prob[8]=asym*nu;//asymmetric switching resource
   }
   prob[1]=dilution*x+mu1/Yx1*x/(x+Kx1)*y1+mu2/Yx2*x/(x+Kx2)*y2;// decrease of  resource
   prob[2]=mu1*x/(x+Kx1)*y1;// increase of consumer 1
   prob[3]=(dilution+d1*z/(z+Kz1))*y1;// decrease of consumer 1
